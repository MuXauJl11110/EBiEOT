...
class LightGCOT(nn.Module):
    def __init__(
        ...
        :param bool is_diagonal: Whether potentials are diagonal, defaults to True
        ...
        self.log_w = nn.Parameter(epsilon * torch.log(torch.ones(n_potentials) / n_potentials))
        ...
        self.log_v = nn.Parameter(epsilon * torch.log(torch.ones(m_potentials) / m_potentials))
