...
def plot_A_parameters(model: LightGCOT, log: bool = False) -> dict[str, wandb.Image] | None:
    ...
    axes[2].scatter(
        model.A_diagonal_matrix[:, 0].cpu().detach().numpy(),
        model.A_diagonal_matrix[:, 1].cpu().detach().numpy(),
        color=color,
    )
    axes[2].set_xlabel("x")
    axes[2].set_ylabel("y")
...
def plot_B_parameters(
    ...
    for color, point in zip(colors_chosen, starting_points):
        ...
        log_v_m = model.compute_log_v_m(point)
        ...
        axes[1].scatter(
            b_m[0, :, 0].cpu().detach().numpy(),
            b_m[0, :, 1].cpu().detach().numpy(),
            label=label,
            color=color,
        )
        axes[1].set_xlabel("x")
...
def plot_G_parameters(
    ...
    for color, point in zip(colors_chosen, starting_points):
        ...
        log_v_m = model.compute_log_v_m(point[None, :])
        b_m = model.compute_b_m(point[None, :])
        B_m = model.compute_B_m(point[None, :])
        ...
        axes[1].scatter(
            g_nm.view(model.m_potentials * model.n_potentials, model.y_dim)[:, 0].cpu().detach().numpy(),
            g_nm.view(model.m_potentials * model.n_potentials, model.y_dim)[:, 1].cpu().detach().numpy(),
            label=label,
            color=color,
        )
        axes[1].set_xlabel("x")
        ...
        axes[2].scatter(
            scale[:, 0].cpu().detach().numpy(),
            scale[:, 1].cpu().detach().numpy(),
            label=label,
            color=color,
        )
        # axes[2].set_xlabel("x")
        ...
        axes[2].set_xscale("log")
        axes[2].set_yscale("log")
        axes[2].set_title(r"$\varepsilon\sqrt{G_{nm}^{-1}}$")
        axes[2].grid(zorder=-20)
