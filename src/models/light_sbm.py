...
class LightSBM(nn.Module):
    def __init__(
        self, dim=2, n_potentials=5, epsilon=1, is_diagonal=True, sampling_batch_size=1, S_diagonal_init=0.1, r_scale=1
    ):
        super().__init__()
        ...
        self.S_rotation_matrix = nn.Parameter(torch.randn(n_potentials, self.dim, self.dim))
    ...
    def forward(self, x):
        ...
        for i in range(num_sampling_iterations):
            ...
            if self.is_diagonal:
                ...
                x_S_x = (sub_batch_x[:, None, None, :] @ (S[None, :, :, :] @ sub_batch_x[:, None, :, None]))[
                    :, :, 0, 0
                ]
                x_r = (sub_batch_x[:, None, :] * r[None, :, :]).sum(dim=-1)
    ...
    def get_drift(self, x, t):
        ...
        A_diagonal = (t / (epsilon * (1 - t)))[:, None, None] + 1 / (epsilon * S_diagonal)[
            None, :, :
        ...
        if self.is_diagonal:
            ...
            c = ((1 / (epsilon * (1 - t)))[:, None] * x)[:, None, :] + (r / (epsilon * S_diagonal))[
                None, :, :
            ]  # shape: batch*potential*dim
            ...
            S_inv = (self.S_rotation_matrix * (1 / S_diagonal[:, None, :])) @ torch.permute(
                self.S_rotation_matrix, (0, 2, 1)
            )
            ...
            c = ((1 / (epsilon * (1 - t)))[:, None] * x)[:, None, :] + (S_inv @ (r[:, :, None]))[
                None, :, :, 0
            ] / epsilon  # shape: batch*potential*dim
        ...
        drift = (
            -x / (1 - t[:, None])
            + epsilon
            * torch.autograd.grad(lse, x, grad_outputs=torch.ones_like(lse, device=lse.device), create_graph=True)[0]
        )
    ...
    def sample_euler_maruyama(self, x, n_steps):
        ...
        for i in range(n_steps):
            x = (
                ...
                + math.sqrt(dt) * torch.sqrt(epsilon) * torch.randn_like(x, device=x.device)
            )
            t += dt
            trajectory.append(x)
