...
class EGEOTBase(ABC):
    ...
    def __init__(self, potential: nn.Module, sample_buffer: SampleBuffer, config: EBMConfig):
        self.potential = potential
        self.sample_buffer = sample_buffer
        self.config = config

    @abstractmethod
    def compute_cost(self, batched_x: torch.Tensor, batched_y: torch.Tensor) -> torch.Tensor:
        raise NotImplementedError()

    def cost_grad_y(self, batched_x: torch.Tensor, batched_y: torch.Tensor) -> torch.Tensor:  # -> [bs x y_dim]
        """
        returns \nabla_y c(x, y)
        """
        # TODO: think about how many times gradient is computing?
        return grad(self.compute_cost, argnums=1)(batched_x, batched_y)
    ...
    def cond_score(
        ...
        cost_part = self.cost_grad_y(x, y) * cost_coeff  # [bs x y_dim]
    ...
    def compute_paired_loss(self, X_paired: torch.Tensor, Y_paired: torch.Tensor) -> torch.Tensor:
        c = self.compute_cost(X_paired, Y_paired)
    ...
    def store(self, path: str):
        ...
        torch.save({"state_dict": self.potential.state_dict(), "config_dict": self.config.model_dump()}, path)
...
class EGEOTl2Sq(EGEOTBase):
    ...
    def compute_cost(self, batched_x: torch.Tensor, batched_y: torch.Tensor) -> torch.Tensor:
        return 0.5 * torch.sum((batched_x - batched_y) ** 2)
...
class EGEOTNoCost(EGEOTBase):
    ...
    def compute_cost(self, batched_x: torch.Tensor, batched_y: torch.Tensor) -> torch.Tensor:
        return torch.tensor(0.0).to(batched_y.device)
