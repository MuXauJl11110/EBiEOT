...
class EGEOT(BaseGenerativeModel):
    ...
    def compute_unpaired_loss(
        ...
        neg_out = self.potential.forward(Y_sampled)
    ...
    def compute_paired_loss(
        ...
        # slightly noise the data
        if self.config.reference_data_noise_sigma > 0.0:
            Y_paired += self.config.reference_data_noise_sigma * torch.randn_like(Y_paired)

        with torch.no_grad():
            if compute_stats:
                Y_sampled, neg_energy_t, cost_t, potential_t, noise_norm = self.get_samples_energy(
                    X_paired, Y_paired, compute_stats=compute_stats
                )
                output = {
                    "neg_energy_t": neg_energy_t,
                    "cost_t": cost_t,
                ...
                Y_sampled = self.get_samples_energy(X_paired, Y_paired, compute_stats=compute_stats)
                output = {}
        cost = self.cost.forward(X_paired, Y_sampled)
        loss = cost.mean()
