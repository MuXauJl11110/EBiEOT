...
def pca_plot(x_0_gt, x_1_gt, x_1_pred, n_plot, step, lims=((-10, 10), (-10, 10))):
    ...
    axes[0].scatter(x_0_gt_pca[:, 0], x_0_gt_pca[:, 1], c="g", edgecolor="black", label=r"$x\sim P_0(x)$", s=30)
    axes[1].scatter(x_1_gt_pca[:, 0], x_1_gt_pca[:, 1], c="orange", edgecolor="black", label=r"$x\sim P_1(x)$", s=30)
    axes[2].scatter(x_1_pred_pca[:, 0], x_1_pred_pca[:, 1], c="yellow", edgecolor="black", label=r"$x\sim T(x)$", s=30)
...
def plot_mapping(independent_mapping, true_mapping, predicted_mapping, target_data, n_plot, step):
    ...
    for i, mapping in enumerate([independent_mapping_pca, true_mapping_pca, predicted_mapping_pca]):
        ...
        axes[i].scatter(inp[:, 0], inp[:, 1], s=s, label=data_label, alpha=data_alpha, zorder=2, color=data_color)
        axes[i].scatter(
            out[:, 0], out[:, 1], s=s, label=mapped_data_label, alpha=data_alpha, zorder=2, color=mapped_data_color
        )

        axes[i].scatter(
            target_data_pca[:1000, 0],
            target_data_pca[:1000, 1],
            c="orange",
            edgecolor="black",
            label=r"$x\sim P_1(x)$",
            s=10,
...
def plot_2D(x_0_gt, x_1_gt, x_1_pred, n_plot, step, lims=((-2.25, 2.5), (-2.25, 2.5))):
    ...
    axes[0].scatter(x_0_gt[:, 0], x_0_gt_pca[:, 1], c="g", edgecolor="black", label=r"$x\sim P_0(x)$", s=16)
    axes[1].scatter(x_1_gt[:, 0], x_1_gt_pca[:, 1], c="orange", edgecolor="black", label=r"$x\sim P_1(x)$", s=16)
    axes[2].scatter(x_1_pred[:, 0], x_1_pred_pca[:, 1], c="yellow", edgecolor="black", label=r"$x\sim T(x)$", s=16)
...
def plot_2D_trajectory(x_0, x_1, trajectory, step, lims=((-2.25, 2.5), (-2.25 / 1.5, 2.5 / 1.5)), x_scatter_alpha=1):
    ...
    axes.scatter(x_0[:, 0], x_0[:, 1], c="g", edgecolor="black", label=r"$x\sim P_0(x)$", s=16, alpha=x_scatter_alpha)
    axes.scatter(x_1[:, 0], x_1[:, 1], c="orange", edgecolor="black", label=r"$x\sim \pi(\cdot|x)$", s=16)
...
def plot_2D_mapping(
    independent_mapping, predicted_mapping, target_data, n_plot, step, lims=((-2.25, 2.5), (-2.25 / 1.5, 2.5 / 1.5))
):
    s = 30
    linewidth = 0.2
    map_alpha = 1
    ...
    independent_mapping_pca = np.concatenate(
        (independent_mapping[:n_plot, :dim], independent_mapping[:n_plot, dim:]), axis=-1
    )
    predicted_mapping_pca = np.concatenate(
        (predicted_mapping[:n_plot, :dim], predicted_mapping[:n_plot, dim:]), axis=-1
    )
