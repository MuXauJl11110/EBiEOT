...
parser = argparse.ArgumentParser(
    description="Evaluating longrun EgEOTs for CMnist2to3", formatter_class=argparse.ArgumentDefaultsHelpFormatter
)
...
cost_grad_dict = {"l2sq": lambda x, y: y - x}
...
def compute_conditional_energy(f, x, y):
    ...
    return c / HREG + (2.0 / (config["epsilon"] ** 2)) * f_y
...
def langevin_grad(epss, start_i=0):
    ...
    for ell, eps in enumerate(epss):
        ...
        if ell == 0 or (ell + 1) % config["log_freq"] == 0 or (ell + 1) == num_steps:
            print(
                "Step {} of {}.   Ave. En={:>14.9f}   Ave. Grad={:>14.9f}".format(
...
epss = np.geomspace(config["epsilon"], config["epsilon"] / 100.0, num=N_DECREASE_SAMPLES)
y_s_t_0 = y_s_t
